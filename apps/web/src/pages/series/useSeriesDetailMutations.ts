...
export function useSeriesDetailMutations({
  ...
  function requireInternalId(): number {
    ...
    if (value === undefined)
      throw new Error('mutation fired before series detail loaded')
    return value
  }
      ...
      queryClient.setQueryData<SeriesDetail>(
        ...
        (old) =>
          ...
          updateEpisodeInDetail(old, episode.id, (ep) => ({
            ...ep,
            watchCount: willWatch
              ? ep.watchCount + 1
              : Math.max(0, ep.watchCount - 1),
  ...
  const bulkUpToHere = useMutation({
    mutationFn: (episodeId: number) =>
      bulkWatch(requireInternalId(), { upToEpisodeId: episodeId }),
    onError: reportError,
    onSettled: invalidate
  })
  ...
  const markSeasonWatched = useMutation({
    mutationFn: (seasonNumber: number) =>
      bulkWatch(requireInternalId(), { seasonNumber }),
    onError: reportError,
    onSettled: invalidate
  })
  ...
  const unwatchSeason = useMutation({
    mutationFn: (seasonNumber: number) =>
      bulkUnwatch(requireInternalId(), { seasonNumber }),
    onError: reportError,
    onSettled: invalidate
  })

  const changeNeedsReview = useMutation({
    mutationFn: (needsReview: boolean) =>
      updateSeries(requireInternalId(), { needsReview }),
    onError: reportError,
    onSettled: invalidate
  })
  ...
  const toggleMute = useMutation<
    ...
    { previous: SeriesDetail | undefined }
  >({
    mutationFn: (pushMuted: boolean) =>
      updateSeries(requireInternalId(), { pushMuted }),
    onMutate: async (pushMuted) => {
      await queryClient.cancelQueries({ queryKey })
      const previous = queryClient.getQueryData<SeriesDetail>(queryKey)
      queryClient.setQueryData<SeriesDetail>(
        queryKey,
        (old) => old && { ...old, pushMuted }
      )
      return { previous }
    },
  ...
  >({
    mutationFn: (favorite: boolean) =>
      updateSeries(requireInternalId(), { favorite }),
    onMutate: async (favorite) => {
      await queryClient.cancelQueries({ queryKey })
      const previous = queryClient.getQueryData<SeriesDetail>(queryKey)
      queryClient.setQueryData<SeriesDetail>(
        queryKey,
        (old) => old && { ...old, favorite }
      )
      return { previous }
    },
