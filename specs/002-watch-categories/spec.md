...
- [ ] All FRs implemented; every E16–E29 decision has at least one test
      asserting it (Article VIII: domain logic coverage).
- [ ] `pnpm lint && pnpm typecheck && pnpm test` green across the workspace.
- [ ] DB migration 0001 maps a seeded v1 `tracking` table correctly
      (proven by test, not by hand).
- [ ] Zip round-trip test green on schemaVersion 2; a v1 zip imports with the
