...
- **FR-045** Episode watch-action redesign (E47): the episode row's "⋮"
  ...
  date / mark as unwatched" sheet. **Shift+click** skips both sheets and
  toggles immediately (unwatch / mark-only-this). A season section
  auto-collapses once complete and swaps its "mark all watched" button for
  a checkbox.
...
| E34 | Segmented progress bar? | New summary field `seasonProgress: { seasons: [{ number, watched, total }], sequential: boolean }` — non-special seasons with ≥1 episode, ordered by number; `watched`/`total` count that season's episodes (total = announced, not just aired) *(SUPERSEDED by 004 E50: counts are aired-only, zero-aired seasons omitted, `sequential` over the aired list — the rendering rules below still stand)*. `sequential` = the watched set forms a contiguous prefix of the (s,e)-ordered non-special episode list (no watched episode after the first unwatched one). Rendering: if `sequential` AND 1 ≤ season count ≤ **12**: seasons with `watched == total` before the frontier → filled square; the frontier season (first with `watched < total`) → mini progress bar filled `watched/total`; seasons after the frontier → hollow square; all seasons full → all filled squares. Otherwise (non-sequential, 0 or >12 seasons) → the existing single percentage bar (watched/aired overall). Applies to SeriesCard and the detail header. |
...
| E47 | Episode-row watch actions after the redesign? | Added post-checklist (2026-07-15), paired with E45. The episode row's "⋮" dropdown (watch again / edit date / mark up to here) is replaced by two checkbox-driven modals, gated on click by watch state: clicking an **unwatched, aired** episode's checkbox — if any earlier episode in the season is unwatched (`hasUnwatchedBefore`, computed in `SeasonSection` from the series' `nextUnwatched` cursor), shows a confirm modal ("mark everything up to here, or just this one?"); otherwise marks it directly (unchanged one-tap behavior). Clicking a **watched** episode's checkbox opens a "watch again / edit date / mark as unwatched" menu (same three actions the old menu had, plus unwatch is now reachable without a separate control). **Amended 2026-07-20 (011):** on desktop that watched menu is a `Modal` popover with `popoverAlign="end-top"` — panel sits to the **left** of the checkbox rail, **top-aligned** with it (sağ üst; not below, not vertically centered); mobile stays a bottom sheet. The unwatched "up to here" confirm remains a centered modal. **Shift+click** on the checkbox (or the ×N rewatch control) skips both confirms and runs `onToggleWatch` immediately — watched → unwatch / remove latest rewatch; unwatched → mark only this episode. Desktop/pointer only; touch keeps the sheet/modal path. `SeasonSection`'s "mark all watched" button becomes a `Checkbox` (checked = season complete, disabled when complete or nothing aired yet); a season now auto-collapses on mount once it's complete (still starts expanded otherwise, season 0 still starts collapsed). <!-- DECISION: 2026-07-19 — power-user shortcut: Shift+click = skip confirm, direct toggle. --><!-- DECISION: 2026-07-20 — watched-options: desktop popover end-top beside checkbox. --> |
