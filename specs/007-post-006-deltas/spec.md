...
5. **Search open-on-select** — *(amended 009)* click opens existing library
   items only; new shows require an explicit "İzlemeye başla" button (adds +
   navigates). ManualListPicker stays retired.
...
| E87 | Search click behavior? | **Amended 009 E131:** clicking a result does **not** add. In-library hits open detail; new shows open `/series/new` (provider preview). **"İzlemeye başla"** on the preview page adds (default list → İzleniyor via E30) and opens detail. ManualListPicker stays retired. |
