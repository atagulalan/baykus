...
| `apps/web` — pages | `SettingsPage.tsx`: two-column, segmented groups, `StepperInput`, region flags/tooltip. `WatchPage.tsx`: double-RAF scroll, sticky section headers, E138 library entry link. `SearchPage.tsx`: `viewTransitionName` on poster. `SeriesDetailPage.tsx`: favorite + rating in overflow menu, icon-only category status, `EpisodeLabel`, `viewTransitionName` on poster. `LibraryPage.tsx` / `AllSeriesPage.tsx`: floating filter FAB (E128). `StatsPage.tsx`: genre translation keys. `CalendarPage.tsx`: no `upcoming` tag in rows; sticky ModeTabs chrome (E133); hide month tab below `sm` (E135); modes as separate routes `/calendar` + `/calendar/month` + `/calendar/schedule` (E136). |
| `apps/web` — Layout.tsx | Transparent sticky header; balanced desktop row of icon-only Watch + Calendar, centered wordmark, icon-only Search + bare `User` profile icon. Desktop Watch opens `/watch`; wordmark keeps the last-view destination. |
| `apps/web` — FilterPanel | E128: single fixed FAB all viewports; sheet/modal form (no desktop top-right button). |
