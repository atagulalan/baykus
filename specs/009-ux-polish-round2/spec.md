...
As a user, `/calendar` timeline opens scrolled to today. (Watch no longer
needs an open-scroll — history left the page for `/watch/history`.)
...
| E114 | Scroll anchor reliability on /watch and /calendar? | **Amended 2026-07-18 (010 WP2):** `/watch` no longer open-scrolls — in-page history moved to `/watch/history`, so the page starts at the top with category sections. Quick-mark still uses `nextHeadingRef` only for E137 scroll compensation. `/calendar` timeline: double-RAF unchanged (E73); scroll-margin amended by E133 so BUGÜN clears the sticky mode chrome as well as the app header. `/calendar` month/schedule modes: no anchor needed (they start at the current month/week). <!-- DECISION: 2026-07-18 — watch open-scroll was only to skip past in-page history; with history on its own route the anchor is dead weight. --> |
...
| E132 | Pull-to-refresh — which screens, what does it trigger? | Touch-only gesture. **Amended by 011 E160:** `/` (library) and `/watch` use the same gesture mechanics to **open `/watch/history`** (History indicator; no sweep) — see E160. Remaining refresh surfaces: `/calendar`, `/calendar/*`, `/user/:handle/all-series`, `/user/:handle/favorites`, and `/watch/history`. On those, pulling down from `scrollY === 0` past the threshold and releasing triggers **the same action as Settings → Data "Refresh all"** (011 E153; was profile button): `startManualSweep` — full (non-staleOnly) refresh of every series, `["library"]` invalidated, existing done/error toasts reused (`library.refreshAllDone` / `errors.generic` — no new i18n keys). After the sweep settles, the gesture additionally awaits invalidation of `["library"]` plus the page's own non-library keys (`["watch-history"]` on /watch/history, `["calendar"]` on /calendar — an E81 "natural refetch", so session-pinned watched rows dropping is correct), so a pull always refetches the visible page even when the sweep guard (quiet sweep or manual refresh already in flight) skipped starting a new one. `startManualSweep` returns the in-flight sweep's settling promise (resolved immediately when guarded) — the Settings button ignores it, behavior unchanged. Gesture mechanics (`PullToRefresh` component wrapping page content): native touch listeners (`passive: false`) on the wrapper; a gesture arms only when the touch starts at document top, then directional-locks (bails if horizontal movement wins — ScheduleGrid/rail panning unaffected, or if a real scroll already started pre-lock); **once locked, every touchmove is consumed until the finger lifts** — upward drift clamps the pull toward 0 and may re-grow, it never hands the gesture back to body scroll mid-flight (device finding 2026-07-17: releasing the preventDefault on upward drift let body scroll yank the page); pull distance dampened (÷2.5, capped ~96px); refresh indicator is a `RefreshCw` icon that rotates with pull progress, turns yellow past the threshold, and spins during refresh with the sweep's mono `done/total` counter beside it; release below threshold animates back and fires nothing. While mounted the component sets `overscroll-behavior-y: contain` on `<html>`/`<body>` so the browser's native pull-to-reload never races the custom gesture; unwrapped pages (search, settings, detail, stats, profile) keep native behavior. No mouse/desktop variant — Settings → Data remains the pointer refresh path. Amends E60: AllSeriesPage still has no refresh button and no auto-sweep; the pull gesture is the one deliberate exception. <!-- DECISION: superseded location prose by 011 E153 — Refresh all lives in Settings → Data; pull-to-refresh still calls the same startManualSweep. --><!-- DECISION: 2026-07-18 — 011 E160 moves `/` + `/watch` to pull-to-history; refresh stays on inner list surfaces. --> |
| E133 | Sticky calendar mode access? | **Amended 2026-07-18:** mode access moves into the already-sticky app navbar; the extra sticky title+ModeTabs chrome is removed. The navbar's top-right action follows the Watch↔Library destination-toggle idiom: timeline shows `GanttChart` → `/calendar/schedule`; schedule shows `List` → `/calendar`; month also points to schedule. Existing `calendar.mode.*` labels provide `aria-label` + `title`. The BUGÜN scroll margin and sticky section-title top offset therefore clear only `--app-header-height`; no `--calendar-mode-chrome-height` or ResizeObserver remains. **Mode change still resets `window.scrollY` to 0** (`behavior: "instant"`) before the new view mounts; re-entering timeline remounts `TimelineView` and re-runs the E73 today anchor. <!-- DECISION: 2026-07-18 product feedback — calendar mode toggle belongs in the app navbar, matching Watch/Library placement and semantics. --> |
...
| E135 | Hide month calendar tab on mobile? | **Amended by E133:** the navbar exposes only the timeline↔schedule destination toggle at every width; month has no dedicated chrome control. `/calendar/month` remains desktop-only and replace-redirects to `/calendar` below `sm`. Rationale: its collapsed mobile list duplicates timeline. |
| E136 | Calendar modes on separate URLs? | Each mode keeps its route: **`/calendar`** = timeline, **`/calendar/month`** = month, **`/calendar/schedule`** = schedule. The E133 navbar action links between timeline and schedule; the nav calendar item uses `exact: false` so all three URLs remain active. Mode change is navigation plus scroll-to-top on mode prop. Pull-to-refresh flush-top applies to `/calendar` and `/calendar/*`. No new i18n keys. |
...
| E141 | Watch per-section sort + add section? | `/watch` only for add/remove; Library / AllSeries also use per-section sort (same `watchSectionSorts` prefs). Default sections: `watching` + `not_watched_recently` (E22). Each sticky section header has a sort control (client-side `sortSeriesSummaries`) and a remove control — **except `watching`, which is pinned and cannot be removed** (remove button hidden; prefs parse always re-inserts it). **`needs_review` is not a user section** (011 E156): auto-prepended when non-empty, hidden when empty, never in **Kategori ekle**, no SortMenu. Bottom of the page: **Kategori ekle** opens a picker of categories not yet on the page (excluding `needs_review`). Section list + per-section sorts persist (E143: settings `ui_prefs` + localStorage cache). Empty *user* sections still render with a hint. No FilterPanel FAB on `/watch`. UI copy uses “kategori” not “bölüm” (episode ambiguity in TR). **Amended 2026-07-18:** `SortMenu` options are category-scoped via `sortsForCategory` — drop no-op keys (`lastWatched` under `not_started` where every item has null `lastWatchedAt`; `nextAir` under `finished`/`stopped`). Stored prefs that violate the allow-list fall back to the category default. **Amended 2026-07-18 (evening):** `needs_review` auto-shows at top when non-empty; never manually addable (see 011 E156). <!-- DECISION: 2026-07-17 — user-composed watch sections beat a global progress filter. --><!-- DECISION: 2026-07-18 — watching stays pinned; other sections remain optional. --><!-- DECISION: 2026-07-18 — section-scoped sort menus omit keys that cannot change order for that category. --><!-- DECISION: 2026-07-18 — needs_review auto when non-empty; hide when empty; never in add picker. --> |
...
- [x] `/watch` opens at top (E114 amended — no open-scroll after history moved off-page); `/calendar` scroll anchor reliable (E73/E114, double-RAF).
