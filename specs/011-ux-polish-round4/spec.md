...
| E157 | Who owns horizontal screen spacing? | `MainShell` is always full-bleed inside its `max-w-5xl` width and provides **no horizontal padding** at any breakpoint. Each visual component owns the inset its content needs: standard page content uses `content-inset` (`12px`, `24px` at `sm+`); list rows and their section headers use the shared tighter row inset (`px-2 sm:px-4`); cinematic media, list borders, calendar schedule rails, settings groups, and other intentionally edge-to-edge surfaces stay flush and pad only their inner text/actions. Negative-margin compensation for a padded shell is forbidden. <!-- DECISION: 2026-07-18 — wrapper padding compounds with row/card padding and makes reusable full-bleed components impossible to align reliably. --> |
